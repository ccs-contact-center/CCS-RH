...
class EntrevistaRH extends Component {
  ...
  rowClick = (e, row) => {
    ...
    this.API_CCS.getCandidato(`${row.getData().id}`).then(res => {
  ...
  handleFormSubmit(e) {
    ...
    this.API_CCS.updateCandidato(this.state)
      .then(res => {
        MySwal.fire({
          title: "¡Correcto!",
  ...
  requestCampanias = async () => {
    const response = await this.API_CCS.getCampanias();
    return response;
  };
